...
def test_generate_worker(
    model_path, tp_rank, tp_size, batch_size, input_len, output_len, nccl_port
):
    ...
    def decode(print_logits, step_idx):
        nonlocal predict_ids
        ...
        logits = model.forward_decode(
            ...
            req_pool_indices,
            seq_lens,
            None,  # prefix_lens is None for decode
            None,  # out_cache_loc
            out_cache_cont_start,
            out_cache_cont_end,
        )
...
def test_generate(model_path, tp_size, batch_size, input_len, output_len):
    # Allocate a free port for NCCL communication
    can_use_ports = alloc_usable_network_port(num=1)
    nccl_port = can_use_ports[0]
...
if __name__ == "__main__":
    test_generate("/data/models/TinyLlama-1.1B-Chat-v0.4", 1, 1, 256, 8)
